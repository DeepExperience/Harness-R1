...
"""Same-batch DBBench delta reward for Harness-R1 code-hook patches."""
...
REWARD_CACHE_PROTOCOL_VERSION = "dbbench_delta_codepatch_v1_20260626"
    ...
    *,
    ...
    for probe in range(span):
        ...
        if not _ports_are_safe_candidates(ports, avoid_ephemeral=avoid_ephemeral):
            continue
        locks = _try_lock_ports(ports)
        if locks is None:
            continue
        if all(_port_is_free(port) for port in ports):
            return controller_port, worker_port_base, locks
        _release_port_locks(locks)
    raise RuntimeError(
        "could not find free DBBench controller/worker ports "
...
def _retryable_eval_failure(log_path: Path, output_root: Path) -> bool:
    ...
    for path in (
        ...
        output_root / "dbbench/batch_000/status.json",
        output_root / "dbbench/batch_000/logs/worker.log",
        output_root / "dbbench/batch_000/logs/assigner.log",
    ):
        try:
...
def _parse_patch(
    ...
    *,
    ...
    try:
        ...
        patch = normalize_patch(raw, bench="dbbench")
...
def _dbbench_runtime_incompatible_reason(patch: dict[str, Any]) -> str:
    for idx, action in enumerate(patch.get("actions") or []):
        if action.get("type") != "add_code_hook":
            return f"action {idx} {action.get('type')!r} is not active in DBBench code-hook RL"
        if action.get("hook") not in {"on_init", "make_pre_hint", "on_before_action", "on_post_step"}:
            return f"action {idx} code hook {action.get('hook')!r} is not executed by DBBench code hooks"
    return ""
...
def _reward_context(
    ...
    *,
    ...
    dbbench_max_round: int,
    dbbench_data_file: str,
    rollout_tool_choice: str,
    rollout_chat_template_kwargs: dict[str, Any],
    auto_retry: bool,
    require_think_patch: bool,
    response_protocol: str,
    dbbench_env_driver: str,
    dbbench_manual_mysql_host: str,
    ...
    return {
        "reward_func": f"reward_dbbench_codepatch.{reward_metric}",
        ...
        "target": {"model": target_model, "urls": target_urls, "agent_name": agent_name},
        "batch": {
            "start": start,
            "end": end,
            "task_ids": task_ids,
            ...
            "metadata_source_rollout_dir": str(metadata.get("source_rollout_dir") or metadata.get("batch_dir") or ""),
        },
        "reward_policy": {
            "threshold": threshold,
        ...
        "rollout": {
            ...
            "dbbench_max_round": dbbench_max_round,
            "dbbench_data_file": dbbench_data_file,
            "dbbench_env_driver": dbbench_env_driver,
            "dbbench_manual_mysql_host": dbbench_manual_mysql_host,
            "startup_timeout": startup_timeout,
            "rollout_timeout": rollout_timeout,
            "reward_timeout": reward_timeout,
        },
        "infra": {
            "controller_port_base": int(getattr(args, "harness_r1_controller_port_base", 12000)),
            "worker_port_base": int(getattr(args, "harness_r1_worker_port_base", 22000)),
            "port_probe_span": int(getattr(args, "harness_r1_port_probe_span", 8000)),
            "avoid_ephemeral_ports": _as_bool(getattr(args, "harness_r1_avoid_ephemeral_ports", True), default=True),
...
def _patched_rewards(eval_output_root: Path) -> dict[int, float]:
    rewards: dict[int, float] = {}
    for path in sorted(eval_output_root.glob("dbbench/batch_*/rollout/*/*/runs.jsonl")):
...
def _score_sync(args: Any, sample: Sample) -> dict[str, Any]:
    ...
    if reject_runtime_noop_patch:
        incompatible_reason = _dbbench_runtime_incompatible_reason(patch)
    ...
    target_model = str(getattr(args, "harness_r1_target_model", metadata.get("target_model", "Qwen3.5-9B")))
    agent_name = str(getattr(args, "harness_r1_agent_name", metadata.get("target_agent_name", "qwen35-9b-dbbench-nothink")))
    task_ids = [int(item) for item in (metadata.get("task_ids") or [])]
    start = int(metadata.get("start") if metadata.get("start") is not None else (min(task_ids) if task_ids else 0))
    end = int(metadata.get("end") if metadata.get("end") is not None else (max(task_ids) + 1 if task_ids else start))
    batch_size = int(metadata.get("batch_size") or (len(task_ids) if task_ids else max(1, end - start)))
    baseline_pass = int(metadata.get("baseline_pass") or 0)
    ...
    reward_timeout = int(getattr(args, "harness_r1_reward_timeout", 3600))
    rollout_max_tokens = int(getattr(args, "harness_r1_rollout_max_tokens", 4096))
    rollout_http_timeout = int(getattr(args, "harness_r1_rollout_http_timeout", 900))
    dbbench_max_round = int(getattr(args, "harness_r1_dbbench_max_round", 20))
    rollout_tool_choice = str(getattr(args, "harness_r1_rollout_tool_choice", "auto") or "auto")
    ...
    default_agentbench_python = (
        ...
        else REPO_ROOT / "code/life-harness/AgentBench/.venv-py310/bin/python"
    )
    agentbench_python = Path(getattr(args, "harness_r1_agentbench_python", default_agentbench_python))
    dbbench_worker_python = Path(getattr(args, "harness_r1_dbbench_worker_python", agentbench_python))
    dbbench_data_file = Path(
        getattr(args, "harness_r1_dbbench_data_file", agentbench_dir / "data/dbbench/db_out_new.jsonl")
    )
    docker_network_name = str(getattr(args, "harness_r1_docker_network_name", "harness_r1_agentbench") or "harness_r1_agentbench")
    dbbench_env_driver = str(getattr(args, "harness_r1_dbbench_env_driver", "docker") or "docker")
    dbbench_manual_mysql_host = str(
        getattr(args, "harness_r1_dbbench_manual_mysql_host", "127.0.0.1") or "127.0.0.1"
    )
    ...
    reward_context = _reward_context(
        ...
        dbbench_max_round=dbbench_max_round,
        dbbench_data_file=str(dbbench_data_file),
    ...
    eval_root = Path(getattr(args, "harness_r1_eval_root", REPO_ROOT / "outputs/relax_harness_r1/reward_cache_dbbench"))
    ...
    try:
        ...
        task_ids_path = eval_dir / "task_ids.json"
        _json_dump(patch_path, patch)
        if task_ids:
            _json_dump(task_ids_path, task_ids)
        _json_dump(
        ...
        run_id = f"relax_dbbench_{batch_tag}_{eval_key}"
        ...
        for attempt in range(eval_max_attempts):
            ...
            controller_port, worker_port_base, port_locks = _select_free_port_pair(
                controller_base=int(getattr(args, "harness_r1_controller_port_base", 12000)),
                worker_base=int(getattr(args, "harness_r1_worker_port_base", 22000)),
                seed=int(eval_key[:8], 16) + attempt * 104729,
                num_workers=1,
                span=int(getattr(args, "harness_r1_port_probe_span", 8000)),
                avoid_ephemeral=_as_bool(getattr(args, "harness_r1_avoid_ephemeral_ports", True), default=True),
            )
            ...
            cmd = [
                ...
                str(dbbench_worker_python),
                "--bench",
                "dbbench",
                "--run-id",
                f"{run_id}_a{attempt}",
                "--dbbench-data-file",
                str(dbbench_data_file),
                "--dbbench-max-round",
                str(dbbench_max_round),
                "--start",
                str(start),
                "--batch-size",
                str(batch_size),
            ...
            patched_rewards = _patched_rewards(output_root)
