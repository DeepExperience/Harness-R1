...
"""Same-batch ALFWorld delta-pass@1 reward for Harness-R1 patches."""
...
REWARD_CACHE_PROTOCOL_VERSION = "alfworld_delta_pass_overlay_v1_20260609"
...
def _ports_are_safe_candidates(ports: list[int], *, avoid_ephemeral: bool = True) -> bool:
...
def _select_free_port_pair(
    *,
    ...
    """Pick and reserve controller/worker ports for this reward subprocess."""
    span = max(1, min(int(span), 60000))
    start = seed % span
    avoid_ephemeral = os.environ.get("HARNESS_R1_AVOID_EPHEMERAL_PORTS", "1").strip().lower() not in {
        "0",
        "false",
        "no",
    }
    ...
    raise RuntimeError(
        "could not find free ALFWorld controller/worker ports "
...
def _retryable_eval_failure(log_path: Path, output_root: Path) -> bool:
    ...
    for path in (
        ...
        output_root / "alfworld/batch_000/status.json",
        output_root / "alfworld/batch_000/logs/worker.log",
    ):
        try:
...
def _relative_batch_rewards(rewards: dict[int, float], *, start: int, batch_size: int) -> dict[int, float]:
    """Normalize task-indexed rewards to batch-local indices.

    Older ALFWorld metadata stores baseline rewards as 0..batch_size-1, while
    some rollout outputs may use absolute task ids. The reward computation below
    needs one index convention so partial reruns can be merged safely.
    """
    ...
    for key, value in rewards.items():
        try:
            ...
            continue
        if 0 <= idx < batch_size:
            normalized[idx] = reward
        elif start <= idx < end:
            normalized[idx - start] = reward
    return normalized
...
def _parse_patch(
    ...
    *,
    ...
    try:
        ...
        elif response_protocol in {"json_patch", "full_think_patch_relaxed", "prefill_think_patch_relaxed"}:
            raw = extract_json_object(response or "")
        else:
            raise PatchValidationError(f"unsupported response protocol: {response_protocol!r}")
        patch = normalize_patch(raw, bench="alfworld")
...
def _alfworld_runtime_incompatible_reason(patch: dict[str, Any]) -> str:
    """Return why a valid patch is an ALFWorld runtime no-op under overlay executors."""
    for idx, action in enumerate(patch.get("actions") or []):
        ...
        if action_type == "add_guard_rule":
            ...
            if trigger != "before_action":
                return f"action {idx} guard trigger {trigger!r} is not executed by ALFWorld overlay guards"
            if effect_kind not in {"block_and_prompt", "force_action", "rewrite_action"}:
                return f"action {idx} guard effect {effect_kind!r} is not executed by ALFWorld overlay guards"
        elif action_type == "add_recovery_rule":
            trigger = action.get("trigger")
            effect_kind = (action.get("effect") or {}).get("kind")
            if trigger != "post_step":
                return f"action {idx} recovery trigger {trigger!r} is not executed by ALFWorld overlay recovery"
            if effect_kind not in {"inject_hint", "force_action"}:
                return f"action {idx} recovery effect {effect_kind!r} is not executed by ALFWorld overlay recovery"
        elif action_type == "add_code_hook":
            if action.get("hook") not in {"on_init", "make_pre_hint", "on_before_action", "on_post_step"}:
                return f"action {idx} code hook {action.get('hook')!r} is not executed by ALFWorld code hooks"
    return ""
...
def _reward_context(
    ...
    *,
    ...
    reward_metric: str,
    allow_partial_eval_reward: bool,
    retry_incomplete_eval: bool,
    eval_max_attempts: int,
) -> dict[str, Any]:
    return {
        "reward_func": f"reward_alfworld_patch.{reward_metric}",
        ...
        "batch": {
            ...
            "batch_size": batch_size,
            "baseline_pass": baseline_pass,
            "split": str(metadata.get("split") or getattr(args, "harness_r1_alfworld_split", "")),
        ...
        "reward_policy": {
            ...
            "allow_partial_eval_reward": allow_partial_eval_reward,
        ...
        "rollout": {
            ...
            "alfworld_max_step": int(getattr(args, "harness_r1_alfworld_max_step", 50)),
            "startup_timeout": startup_timeout,
            "rollout_timeout": rollout_timeout,
            "reward_timeout": reward_timeout,
            "controller_port_base": int(getattr(args, "harness_r1_controller_port_base", 61200)),
            "worker_port_base": int(getattr(args, "harness_r1_worker_port_base", 64200)),
            "port_probe_span": int(getattr(args, "harness_r1_port_probe_span", 800)),
            "eval_retry_on_infra_failure": int(getattr(args, "harness_r1_eval_retry_on_infra_failure", 1) or 0),
            "eval_max_attempts": eval_max_attempts,
        },
    }


def _patched_rewards(eval_output_root: Path) -> dict[int, float]:
    rewards: dict[int, float] = {}
    for path in sorted(eval_output_root.glob("alfworld/batch_*/rollout/*/*/runs.jsonl")):
...
def _score_sync(args: Any, sample: Sample) -> dict[str, Any]:
    ...
    if reject_runtime_noop_patch:
        incompatible_reason = _alfworld_runtime_incompatible_reason(patch)
    ...
    target_urls = _as_list(getattr(args, "harness_r1_target_urls", None)) or [
        str(getattr(args, "harness_r1_target_url", "http://127.0.0.1:8001/v1"))
    ]
    target_model = str(getattr(args, "harness_r1_target_model", metadata.get("target_model", "Qwen2.5-7B-Instruct")))
    agent_name = str(getattr(args, "harness_r1_agent_name", metadata.get("target_agent_name", "alfworld-target")))
    ...
    rollout_max_tokens = int(getattr(args, "harness_r1_rollout_max_tokens", 4096))
    rollout_http_timeout = int(getattr(args, "harness_r1_rollout_http_timeout", 120))
    rollout_tool_choice = str(getattr(args, "harness_r1_rollout_tool_choice", "required") or "required")
    rollout_chat_template_kwargs = getattr(args, "harness_r1_rollout_chat_template_kwargs", {}) or {}
    if isinstance(rollout_chat_template_kwargs, str):
        rollout_chat_template_kwargs = json.loads(rollout_chat_template_kwargs) if rollout_chat_template_kwargs else {}
    ...
    allow_partial_eval_reward = bool(getattr(args, "harness_r1_allow_partial_eval_reward", False))
    retry_incomplete_eval = _as_bool(getattr(args, "harness_r1_retry_incomplete_eval", False))
    retry_count = int(getattr(args, "harness_r1_eval_retry_on_infra_failure", 1) or 0)
    eval_max_attempts = max(
        1,
        int(getattr(args, "harness_r1_eval_max_attempts", retry_count + 1) or 1),
        retry_count + 1,
    )
    ...
    eval_root = Path(
        getattr(args, "harness_r1_eval_root", REPO_ROOT / "outputs/relax_harness_r1/reward_cache_alfworld")
    )
    ...
    if not lock_owner and reward_path.exists():
        ...
        return result
    if not lock_owner:
        return _attach_timing(_no_patch_reward(metadata, "cache_lock_timeout"), _finish_timing(timing, t0))
    ...
    try:
        ...
        run_id = f"relax_alfworld_{batch_tag}_{eval_key}"
        ...
        agentbench_python = Path(getattr(args, "harness_r1_agentbench_python", default_agentbench_python))
        alfworld_worker_python = Path(
            getattr(
                args,
                "harness_r1_alfworld_worker_python",
                agentbench_dir / ".venvs/alfworld-worker/bin/python",
            )
        )
        alfworld_split = str(metadata.get("split") or getattr(args, "harness_r1_alfworld_split", "harness_r1_eval500_20260609"))
        ...
        patched_rewards_raw: dict[int, float] = {}
        patched_n = 0
        ...
        for attempt in range(max_eval_attempts):
            ...
            controller_port, worker_port_base, port_locks = _select_free_port_pair(
                controller_base=int(getattr(args, "harness_r1_controller_port_base", 61200)),
                worker_base=int(getattr(args, "harness_r1_worker_port_base", 64200)),
                seed=int(eval_key[:8], 16) + attempt * 104729,
                num_workers=1,
                span=int(getattr(args, "harness_r1_port_probe_span", 800)),
            )
            ...
            cmd = [
                ...
                alfworld_split,
                "--alfworld-max-step",
                str(int(getattr(args, "harness_r1_alfworld_max_step", 50))),
                "--alfworld-worker-python",
                str(alfworld_worker_python),
                "--start",
                str(start),
                "--batch-size",
                str(batch_size),
            ...
            if rollout_chat_template_kwargs:
                cmd.extend(["--rollout-chat-template-kwargs", json.dumps(rollout_chat_template_kwargs)])
            if auto_retry:
            ...
            patched_rewards_raw = _patched_rewards(output_root)
            patched_rewards = _relative_batch_rewards(patched_rewards_raw, start=start, batch_size=batch_size)
            patched_n = len(patched_rewards)
        ...
        baseline_rewards = _relative_batch_rewards(_baseline_rewards(metadata), start=start, batch_size=batch_size)
        patched_pass = sum(value >= threshold for value in patched_rewards.values())
        baseline_average_reward = _baseline_average_reward(metadata, batch_size=batch_size)
        ...
        if proc is None:
            raise RuntimeError("ALFWorld eval subprocess was not executed")
        if proc.returncode != 0 or patched_n < batch_size:
            if allow_partial_eval_reward and patched_n > 0 and baseline_rewards:
                expected = set(range(batch_size))
                completed = {idx: value for idx, value in patched_rewards.items() if idx in expected}
                merged_rewards = {idx: baseline_rewards.get(idx, 0.0) for idx in range(batch_size)}
                merged_rewards.update(completed)
                missing_indices = sorted(expected - set(completed))
                patched_pass = sum(value >= threshold for value in merged_rewards.values())
                patched_average_reward = _average_reward(merged_rewards, batch_size=batch_size)
                delta_average_reward = patched_average_reward - baseline_average_reward
                ...
                result = {
                    ...
                    "valid_patch": True,
                    "eval_status": "partial_eval_used_baseline_for_missing",
                    "detail": f"returncode={proc.returncode}; patched_n={patched_n}; log={log_path}",
                    "patch_path": str(patch_path),
                    "eval_log": str(log_path),
                    "target_url": target_url,
                    "patched_rewards": merged_rewards,
                    "completed_patched_rewards": completed,
                    "raw_patched_rewards": patched_rewards_raw,
                }
            else:
                result = _no_patch_reward(
                    metadata,
                    "eval_failed_treated_as_no_patch",
                    f"returncode={proc.returncode}; patched_n={patched_n}; log={log_path}",
                )
                result.update(
                    {
                        "valid_patch": True,
                        ...
                        "eval_log": str(log_path),
                        "valid_bonus": 0.0,
                        "configured_valid_bonus": valid_bonus,
                    }
                )
        else:
            delta_pass = patched_pass - baseline_pass
