...
def main(args):
    ...
    if not ray.is_initialized():
        ...
        init_runtime_env = runtime_env
        if os.getenv("RELAX_RAY_INIT_RUNTIME_ENV", "1") == "0":
            logger.info("Skipping runtime_env during ray.init; it is still passed to controller actors.")
            init_runtime_env = None
        ray.init(runtime_env=init_runtime_env)
        logger.info("Ray initialized successfully")
        try:
            serve.start(
                http_options={"host": "0.0.0.0", "port": int(os.getenv("RELAX_SERVE_PORT", "8000"))},
                detached=True,
            )
        except RuntimeError:
            pass
