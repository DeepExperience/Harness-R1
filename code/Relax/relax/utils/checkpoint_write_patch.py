...
def patch_checkpoint_write():
    ...
    """
    _patch_nvrx_filesystem_async_compat()
    _prefer_mcore_checkpoint_strategy()
...
def _patch_nvrx_filesystem_async_compat():
    """Patch small NVRx API drift used by this Megatron checkout.

    Some installed nvidia-resiliency-ext builds expose the helper as
    ``_get_write_results_queue`` while this Megatron version imports the public
    name ``get_write_results_queue``.
    """
    try:
        import nvidia_resiliency_ext.checkpointing.async_ckpt.filesystem_async as filesystem_async
    ...
    if not hasattr(filesystem_async, "get_write_results_queue") and hasattr(
        ...
        logger.info("Patched nvidia_resiliency_ext filesystem_async.get_write_results_queue alias")


def _prefer_mcore_checkpoint_strategy():
    """Use Megatron Core's checkpoint writer over the installed NVRx build.

    The local nvidia-resiliency-ext package is importable, so Megatron's sync
    save path selects NVRx automatically. That package version is not API
    compatible with this Megatron checkout, while the bundled mcore writer is.
    """
    try:
    ...
    if getattr(torch_strategy, "HAVE_NVRX", False):
        torch_strategy.HAVE_NVRX = False
        logger.info("Disabled NVRx checkpoint strategy; using Megatron Core checkpoint writer")
