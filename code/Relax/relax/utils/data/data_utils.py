...
def filter_long_prompts(
    ...
    if processor:
        ...
        if actual_workers > 1 and len(samples) > 1:
            filtered_samples_by_index: list[Sample | None] = [None] * len(samples)
            with ProcessPoolExecutor(
                ...
                futures = {
                    executor.submit(_check_sample_length_worker, sample, max_length): index
                ...
                for future in tqdm.tqdm(
                    ...
                    try:
                        ...
                        if is_valid:
                            filtered_samples_by_index[index] = sample
                        else:
                            logger.debug(f"Filtered sample exceeding max_length={max_length}")
                    except Exception as e:
                        logger.warning(f"Error checking sample length: {e}")
                        filtered_samples_by_index[index] = samples[index]
            # as_completed() yields worker completion order, which must not
            # silently reorder rollout prompts when rollout_shuffle is false.
            filtered_samples = [sample for sample in filtered_samples_by_index if sample is not None]
        else:
            filtered_samples = []
