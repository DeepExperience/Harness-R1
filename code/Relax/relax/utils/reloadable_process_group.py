...
def monkey_patch_torch_dist(args=None):
    ...
    old_get_world_size = dist.get_world_size
    ...
    def get_new_all_reduce_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
            ...
            try:
                world_size = old_get_world_size(group)
            except Exception:
                world_size = None
            ...
            if world_size == 1:
                ...
                if len(args) >= 4:
                    async_op = args[3]
                return _CompletedNoopWork() if async_op else None
    ...
    def get_new_all_gather_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
            ...
            if world_size == 1:
                tensor_list = args[0] if len(args) >= 1 else None
                tensor = args[1] if len(args) >= 2 else None
                if tensor_list is not None and tensor is not None and len(tensor_list) > 0:
    ...
    def get_new_all_gather_into_tensor_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
            ...
            if world_size == 1:
                output_tensor = args[0] if len(args) >= 1 else None
                input_tensor = args[1] if len(args) >= 2 else None
                if output_tensor is not None and input_tensor is not None:
    ...
    def get_new_all_gather_object_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
            ...
            if world_size == 1:
                object_list = args[0] if len(args) >= 1 else None
    ...
    def get_new_gather_object_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
            ...
            if world_size == 1:
                obj = args[0] if len(args) >= 1 else kwargs.get("obj")
                object_gather_list = kwargs.get("object_gather_list")
                if len(args) >= 2:
                    object_gather_list = args[1]
    ...
    def get_new_broadcast_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
    ...
    def get_new_broadcast_object_list_function(func):
        def new_function(*args, **kwargs):
            args = tuple([arg.group if isinstance(arg, ReloadableProcessGroup) else arg for arg in args])
            kwargs = {k: (v.group if isinstance(v, ReloadableProcessGroup) else v) for k, v in kwargs.items()}
    ...
    dist.all_reduce = get_new_all_reduce_function(dist.all_reduce)
    dist.all_gather = get_new_all_gather_function(dist.all_gather)
    dist.all_gather_into_tensor = get_new_all_gather_into_tensor_function(dist.all_gather_into_tensor)
    dist.all_gather_object = get_new_all_gather_object_function(dist.all_gather_object)
    dist.gather_object = get_new_gather_object_function(dist.gather_object)
    dist.all_to_all = get_new_comm_function(dist.all_to_all)
    dist.all_to_all_single = get_new_comm_function(dist.all_to_all_single)
    dist.broadcast = get_new_broadcast_function(dist.broadcast)
    ...
    dist.broadcast_object_list = get_new_broadcast_object_list_function(dist.broadcast_object_list)
