...
def post_process_env(args, env):
    ...
    passthrough_env_keys = [
        ...
        "RELAX_MASTER_PORT_BASE",
        "RELAX_MASTER_PORT_BASE_ACTOR",
        "RELAX_MASTER_PORT_BASE_ACTOR_FWD",
        "RELAX_MASTER_PORT_BASE_REFERENCE",
        "RELAX_MASTER_PORT_BASE_CRITIC",
        "RELAX_ROLLOUT_UPDATE_PORT_MIN",
        "RELAX_ROLLOUT_UPDATE_PORT_MAX",
        "RELAX_ACTOR_REF_UPDATE_PORT_MIN",
        "RELAX_ACTOR_REF_UPDATE_PORT_MAX",
        "RELAX_NCCL_GROUP_DESTROY_WAIT_SEC",
        "RELAX_SERVE_PORT",
...
def merge_dict_list(dict_list):
    ...
    if "response_lengths" not in merged and "loss_masks" in merged:
        merged["response_lengths"] = [int(len(mask)) for mask in merged["loss_masks"]]
    return merged
...
def get_serve_url(route_prefix: str = "") -> str:
    ...
    serve_url = f"http://{head_ip}:{int(os.getenv('RELAX_SERVE_PORT', '8000'))}{route_prefix}"
...
def recovery_load_path(args: Namespace) -> Optional[str]:
    ...
    if args.save is not None and os.path.exists(os.path.join(args.save, "latest_checkpointed_iteration.txt")):
        # Keep model-only checkpoint semantics intact. Some Harness-R1 runs
        # intentionally use --no-save-optim; forcing optimizer load during
        # recovery then crashes with KeyError("optimizer").
