    ...
    async def run(self) -> None:
        ...
        try:
            while step < self.config.num_rollout:
                ...
                chunks_per_step = max(1, int(getattr(self.config, "num_iters_per_train_update", 1)))
                batch_size = self.config.global_batch_size // chunks_per_step
                processed_chunks = 0
                empty_polls = 0
                while processed_chunks < chunks_per_step:
                    adv_data_fields = [
                        "tokens",
                        "total_lengths",
                        "response_lengths",
                    ...
                    if batch_meta.size == 0:
                        empty_polls += 1
                        if empty_polls % 300 == 0:
                            self._logger.info(
                                "Advantage step %s waiting for chunk %s/%s from transfer queue.",
                                step,
                                processed_chunks + 1,
                                chunks_per_step,
                            )
                    ...
                    self._logger.info(
                        f"Successfully got rollout_id: {step} chunk {processed_chunks + 1}/{chunks_per_step} "
                        "data from transfer queue for compute advantages and returns."
                    )
