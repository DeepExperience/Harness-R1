...
def _master_port_start_for_role(role):
    role_name = _role_value(role)
    env_key = f"RELAX_MASTER_PORT_BASE_{role_name.upper()}"
    if env_key in os.environ:
        return int(os.environ[env_key])
    if "RELAX_MASTER_PORT_BASE" in os.environ:
        return int(os.environ["RELAX_MASTER_PORT_BASE"])
    return _ROLE_MASTER_PORT_BASES.get(role_name, 28000)
...
class TrainRayActor(RayActor):
    def __init__(self, world_size, rank, master_addr, master_port, lock, role="actor"):
        ...
        if master_addr:
            ...
            # Fully-async creates actor/reference/actor_fwd services in
            # parallel. Each service has an independent torch process group,
            # so reserve non-overlapping search ranges to avoid a race where
            # multiple rank-0 actors pick the same free-but-not-yet-bound port.
            self.master_addr, self.master_port = self._get_current_node_ip_and_free_port(
                start_port=_master_port_start_for_role(self.role) + random.randint(0, 99)
            )
        ...
        logger.info(
            "TrainRayActor distributed env: role=%s rank=%s/%s master=%s:%s local_rank=%s ray_gpu_ids=%s",
            self.role,
            self._rank,
            self._world_size,
            self.master_addr,
