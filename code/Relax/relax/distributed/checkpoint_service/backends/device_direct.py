...
class DeviceDirectBackend(CommBackend):
    ...
    def __init__(
        ...
        *,
        ...
        self._rollout_update_topology_signature = None
        self._model_update_groups_for_actor_fwd_ref = None
        self._actor_fwd_ref_has_receivers = False
    ...
    def _rollout_topology_signature(self, topology: Dict) -> tuple:
        default_gpus = self.args.rollout_num_gpus_per_engine
        signature = []
        for rank, role_info in sorted(topology.items(), key=lambda kv: int(kv[0])):
            metadata = role_info.get("metadata") if isinstance(role_info, dict) else {}
            signature.append(
                (
                    int(rank),
                    role_info.get("ip") if isinstance(role_info, dict) else None,
                    role_info.get("port") if isinstance(role_info, dict) else None,
                    (metadata or {}).get("num_gpus_per_engine", default_gpus),
                )
            )
        return tuple(signature)

    def _destroy_rollout_update_group(self) -> None:
        """Destroy the temporary actor-rollout NCCL group after weight sync."""
        ...
        try:
            ...
            logger.info("Destroyed local rollout weight update process group %s", group_name)
        except Exception as e:
            logger.warning("Error destroying local rollout weight update process group %s: %s", group_name, e)
        finally:
            self._model_update_groups = None
            self._rollout_update_topology_signature = None
        ...
        try:
            ...
            logger.info("Destroyed remote rollout weight update process group %s", group_name)
        except Exception as e:
            logger.warning("Error destroying remote rollout weight update process group %s: %s", group_name, e)

        time.sleep(float(os.getenv("RELAX_NCCL_GROUP_DESTROY_WAIT_SEC", "1.0")))
    ...
    _MASTER_PORT_MIN = int(os.getenv("RELAX_ROLLOUT_UPDATE_PORT_MIN", "55000"))
    _MASTER_PORT_MAX = int(os.getenv("RELAX_ROLLOUT_UPDATE_PORT_MAX", "55999"))
    ...
    def init_process_group_for_rollout(self, topology_data: Optional[Dict] = None) -> None:
        ...
        if self._is_pp_src_rank:
            ...
            topology_signature = self._rollout_topology_signature(self.rollout_topology)

            if self._model_update_groups is not None and self._rollout_update_topology_signature == topology_signature:
                if not self.rollout_engines:
                    self._create_rollout_engines(self.rollout_topology)
                self._update_rollout_engines()
                logger.info(
                    "Reusing rollout weight update process group %s for topology %s",
                    self._group_name,
                    topology_signature,
                )
                return
            ...
            if self._model_update_groups is not None:
                logger.info("Destroying stale rollout weight update process group before re-init")
                self._destroy_rollout_update_group()
            ...
            for attempt in range(1, max_retries + 1):
                ...
                try:
                    ...
                    ray.get(futures)
                    self._rollout_update_topology_signature = topology_signature
                    last_error = None
                    break
                except Exception as e:
                    last_error = e
    ...
    def init_process_groups_for_actor_fwd_ref(self, topology_data) -> None:
        ...
        self._actor_fwd_ref_has_receivers = world_size > 1
        if not self._actor_fwd_ref_has_receivers:
            logger.info(
                "Skipping actor_fwd_ref weight update process group for role %s: "
                "no receiver ranks (world_size=%s)",
                self.role_info.role_name,
                world_size,
            )
            self._model_update_groups_for_actor_fwd_ref = None
            return
    ...
    def update_weights_for_rollout(self, rollout_only=False, actor_fwd_only=False) -> None:
        ...
        if not rollout_only:
            if dist.get_rank() == 0:
                if self._model_update_groups_for_actor_fwd_ref is not None:
                    ...
                    logger.info("Skipping actor_fwd_ref final weight marker: no process group initialized")
            dist.barrier(group=get_gloo_group())
        ...
        if not actor_fwd_only:
            ...
            dist.barrier(group=get_gloo_group())
            destroy_each_step_env = os.getenv("RELAX_DESTROY_ROLLOUT_UPDATE_GROUP_EACH_STEP")
            if destroy_each_step_env is None:
                destroy_each_step = not bool(getattr(self.args, "fully_async", False))
            else:
                destroy_each_step = destroy_each_step_env.lower() in ("1", "true", "yes", "on")
            if destroy_each_step:
                self._destroy_rollout_update_group()
                self._cleanup_rollout_engines()
            else:
                group_name = getattr(self, "_group_name", None)
                if group_name is not None:
                    logger.info("Keeping rollout weight update process group %s for reuse", group_name)
    ...
    def _update_bucket_weights_from_distributed(
        ...
        try:
            ...
            logger.warning("Rollout distributed weight update failed; destroying rollout update group", exc_info=True)
            self._destroy_rollout_update_group()
            self._cleanup_rollout_engines()
            raise
        finally:
    ...
    def _update_bucket_weights_from_distributed_for_actor_fwd_ref(
        ...
        """
        if self._model_update_groups_for_actor_fwd_ref is None:
            logger.debug("Skipping actor_fwd_ref bucket update: no process group initialized")
            return
