...
def log_rollout_data(
    ...
    """
    if "response_lengths" not in rollout_data and "loss_masks" in rollout_data:
        rollout_data["response_lengths"] = [
            int(mask.numel()) if hasattr(mask, "numel") else int(len(mask)) for mask in rollout_data["loss_masks"]
        ]
    missing_required = [key for key in ("response_lengths", "loss_masks", "total_lengths") if key not in rollout_data]
    if missing_required:
        logger.warning(
            "Skip rollout logging for rollout_id=%s because merged train buffer is missing fields: %s; available fields: %s",
            rollout_id,
            missing_required,
            sorted(rollout_data.keys()),
        )
        return
