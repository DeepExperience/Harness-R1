...
def _convert_mtp_layer(args, name, param, layer_idx):
    ...
    inner_layer_prefix = None
    if "transformer_layer" in name:
        inner_layer_prefix = f"mtp.layers.{layer_idx}.transformer_layer"
    elif "mtp_model_layer" in name:
        inner_layer_prefix = f"mtp.layers.{layer_idx}.mtp_model_layer"

    if inner_layer_prefix is not None:
        proxy_name = name.replace(inner_layer_prefix, f"decoder.layers.{layer_idx}")
