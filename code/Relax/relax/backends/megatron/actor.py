...
def _find_free_nccl_port(port_min: int, port_max: int) -> int:
    ...
    for port in ports:
        try:
            ...
            continue
    raise RuntimeError(f"Could not find a free NCCL port in range [{port_min}, {port_max}]")
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _init(
        ...
        if use_tensor_backuper:
            ...
            if is_pp_src_rank:
                ...
                master_port = _find_free_nccl_port(
                    int(os.getenv("RELAX_ACTOR_REF_UPDATE_PORT_MIN", "56000")),
                    int(os.getenv("RELAX_ACTOR_REF_UPDATE_PORT_MAX", "56999")),
                )
            else:
                master_address = None
                master_port = None
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
        ...
        if "total_lengths" in rollout_data:
            ...
            logger.warning(
                "Skip seq_lens perf aggregation for rollout_id=%s because merged train buffer lacks total_lengths; "
                "available fields: %s",
                rollout_id,
